...
> final validation. The dataset is also synthetic, with labels generated by
> deterministic rules over these features, so near-perfect scores are expected
> and would not transfer to real-sensor data — see
...
- **The dataset is synthetic, and the strong scores partly reflect that.**
  In AI4I 2020 the failure labels are generated by deterministic rules over the
  very quantities this project engineers as features: heat-dissipation failure
  uses `Temperature difference` and rotational speed, power failure uses
  `Power [W]`, and overstrain uses `Tool wear × Torque` (the `OSF criterion`).
  A model that recovers those rules will look near-perfect (ROC-AUC ≈ 0.99), but
  that ceiling is a property of the simulator, not evidence the approach would
  transfer to noisy real-sensor data. This is a **methodology demonstration,
  not a deployment-ready predictor.** Because every record comes from the same
  generator, even a "future batch" from this dataset would follow the same
  rules — genuine external validation needs data from real machines.
